  ...
  return (
    <div className="relative flex-1 min-w-[180px]" ref={wrapRef}>
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 mt-1 w-full bg-surface border border-slate-200 dark:border-slate-600 rounded-xl shadow-lg max-h-40 overflow-y-auto">
          {filtered.map(o => (
            <button
              key={o.id}
              type="button"
              ...
              className="w-full text-right px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm text-slate-900 dark:text-slate-100"
            >
              {o.label}{o.sub ? ` — ${o.sub}` : ''}
            </button>
          ))}
