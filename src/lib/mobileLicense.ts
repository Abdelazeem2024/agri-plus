/**
 * التحقق من ترخيص الهاتف — Ed25519 (منتج مستقل خاص بالهاتف: PROD-006)
 * ==========================================================================
 * هذا الملف "تحقق فقط" (Verify Only) — لا يحتوي على أي مفتاح خاص، فقط
 * المفتاح العام (Public Key) الخاص بمنتج "Agri Plus موبايل" (PROD-006)،
 * المسجَّل بشكل مستقل تماماً في أداة التفعيل عن منتج سطح المكتب (PROD-002).
 * ترخيص سطح المكتب لن يعمل إطلاقاً على الهاتف والعكس صحيح — كل منتج معزول
 * بمفتاحه الخاص كما هو مصمَّم في أداة التفعيل نفسها.
 *
 * تحققت من استخراج المفتاح الخام (32 بايت) من ملف PEM الذي أرسلته بايتاً
 * بايت قبل كتابة هذا الملف (نفس الرأس الثابت المعياري لمفاتيح Ed25519
 * بصيغة SPKI: 302a300506032b6570032100 + 32 بايت المفتاح الخام).
 *
 ...
 *
 * ⚠️ إن ولَّدت مفتاحاً جديداً لهذا المنتج لاحقاً (تجديد/استبدال)، يجب تحديث
 * PUBLIC_KEY_HEX هنا فوراً بالمثل.
 */
import { verifyAsync } from '@noble/ed25519';

const PRODUCT_ID = 'PROD-006';
...
const PUBLIC_KEY_HEX = 'b9bac78c7a4de3cd38eaf2301ed4cf3bb1b4e34f65303ec95f8c601e5f611343';
