 ...
 * إنترنت وبدون أي نموذج توليدي ضخم. مصمَّم خصيصاً لمفردات محلات المبيدات
 * الزراعية. الفلسفة: "الفهم قبل التنفيذ" — إذا لم يكن المحرك واثقاً، يطلب
 * توضيحاً بدل التخمين.
 */
...
export function fuzzyFind<T extends { name: string }>(query: string, list: T[], threshold = 0.55): T | null {
...
function extractDateRange(text: string): { from: string; to: string; label: string } | null {
  ...
  if (/(اليوم|النهارده|النهاردة)/.test(n)) return { from: fmt(today), to: fmt(today), label: 'اليوم' };
...
export type IntentType =
  | 'profit_summary' | 'customer_debt' | 'customers_with_debt' | 'sales_summary'
  | 'collections_summary' | 'low_stock' | 'top_customers' | 'product_stock' | 'unknown';

interface IntentPattern { type: IntentType; keywords: string[][]; }
...
export function matchIntent(text: string): { intent: IntentType; personQuery?: string } {
  const n = normalizeArabic(text);
...
export function understandAndExecute(rawText: string, data: AppData): AIResult {
  ...
  switch (intent) {
    ...
    case 'customer_debt': {
      ...
      if (customer && (!rep || normalizeArabic(customer.name).includes(normalizeArabic(query)))) {
        ...
        return {
          ...
          summary: balance > 0 ? `العميل ${customer.name} عليه مديونية قدرها ${money(balance)} جنيه.` : `لا توجد مديونية على العميل ${customer.name} حالياً.`,
          stats: [
      ...
      return {
        ...
        summary: balance > 0 ? `المحل مديون للمندوب ${repFound.name} بمبلغ ${money(balance)} جنيه.` : `لا توجد مديونية على المحل تجاه المندوب ${repFound.name}.`,
        stats: [
    ...
    case 'low_stock': {
      const rows = data.products.filter(p => p.currentStock <= p.minStock).sort((a, b) => a.currentStock - b.currentStock);
      if (rows.length === 0) {
    ...
    default:
      return {
        understood: false, intent: 'unknown',
